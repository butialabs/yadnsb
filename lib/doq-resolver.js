...
try {
    const quic = await import('@matrixai/quic');
    QUICClient = quic.QUICClient;
} catch (error) {
    console.warn(`[DoQ] Native QUIC binding unavailable on ${process.platform}/${process.arch}: ${error.message}`);
}
...
class UnsupportedDoQResolver {
    constructor() {
        this.timeout = 5000;
    }

    async resolve(domain, server, type = 'A') {
        const message = `DoQ is not supported on ${process.platform}/${process.arch}`;
        console.warn(`[DoQ] ${message}`);
        return {
            success: false,
            responseTime: 0,
            error: message,
...
export default QUICClient ? DoQResolver : UnsupportedDoQResolver;
